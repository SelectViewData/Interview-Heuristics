module internal FSharp.SystemTextJson.OpenApi.AbstractSubtypes
...
let isArrayOfGenericType (t: Type) (tGeneric: Type) =
    ...
    && let elememtType = t.GetElementType() in
       elememtType |> Option.ofObj |> Option.exists(fun elememtType -> 
...
let (|GenericInherit|_|) (tToCompare: Type) (t: Type) =
    ...
    | null -> None
    | baseType ->
        if baseType.IsGenericType && baseType.GetGenericTypeDefinition() = tToCompare then
            Some(baseType.GenericTypeArguments)
        else
            None
