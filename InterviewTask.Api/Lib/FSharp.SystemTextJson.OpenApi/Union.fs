module internal FSharp.SystemTextJson.OpenApi.Union
...
let createDataContractForInternalTag
    ...
    if fsOptions.UnionEncoding.HasFlag JsonUnionEncoding.NamedFields then
        ...
        // we want to have a flattened object {kind, recordProp1, recordProp2, etc.}        
        if fields.Length = 1 && FSharpType.IsRecord fields[0] && fsOptions.UnionEncoding.HasFlag JsonUnionEncoding.UnwrapRecordCases then
          let properties = Record.getRecordDataProperties fields[0] fsOptions options
          let allProps =
            seq {
                yield caseProperty
                yield! properties
          ...
          DataContract.ForObject(typeCase, allProps, jsonConverter = Helper.getJsonConverterFunc options)
    ...
    let fields =
        ...
        |> Array.map (fun (p, name, nameIndex) ->
            ...
            let names =
                ...
                | false, _ ->
                    // not sure if that value in FSharp.SystemTextJson can be null 
                    let unionFieldPolicy: JsonNamingPolicy | null = fsOptions.UnionFieldNamingPolicy
                    let policy =
                        match unionFieldPolicy with
