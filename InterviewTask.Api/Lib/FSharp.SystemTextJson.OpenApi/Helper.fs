module internal FSharp.SystemTextJson.OpenApi.Helper
...
let getJsonNames kind (getAttributes: Type -> obj[]) =
    match
        ...
        |> Array.choose (function
            | :? JsonNameAttribute as attr ->
              // not sure if that value in FSharp.SystemTextJson can be null
              let field: string | null = attr.Field
              if isNull field then Some attr else None
            | _ -> None)
    with
    | [||] ->
    ...
    | _ ->
        failf
          $"To provide multiple names for the same %s{kind}, use a single JsonNameAttribute with multiple arguments"


let convertName (policy: JsonNamingPolicy | null) (name: string) =
    match policy with
    | null -> name
...
let internal getGenericType (typeToConvert: Type) = typeToConvert.GetGenericArguments()[0]


let customOptions (t: Type) =
    match t.GetCustomAttributes(typedefof<JsonFSharpConverterAttribute>, false) with
    | [| atr |] -> atr :?> IJsonFSharpConverterAttribute |> (fun d -> d.Options) |> Some
    | _ -> None
