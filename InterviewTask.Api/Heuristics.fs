...
let score (weights: Weights) (crews: Crew list) (assignments: Assignment list) : ScoreBreakdown =
  ...
  let perCrew =
    ...
    |> List.map (fun a ->
      ...
      let overtimeHours = max 0 (assignedHours - crew.maxHoursPerDay)

      // BUG (intentional for interview): when overtimeHours == overtimeCap, we under-penalize by 1 hour.
      let overtimeHoursForPenalty =
        if overtimeHours = crew.overtimeCap then
          max 0 (overtimeHours - 1)
        else
          overtimeHours
      ...
      let overtimePenalty = overtimeHoursForPenalty * weights.overtimeWeight
